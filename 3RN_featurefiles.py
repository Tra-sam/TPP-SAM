...
#Road negtive feature points to feature points files
...
# Output the centroids to a CSV file (append mode)
def output_centers(file_path, medoids, data, output_folder):
    ...
    df_centroids["L"] = ["RN"] * len(centroids)  # All rows filled with "RN"

    # Append the DataFrame to the existing CSV file
    if os.path.exists(output_file_path):
        df_centroids.to_csv(output_file_path, mode='a', header=False, index=False)
    else:
        df_centroids.to_csv(output_file_path, mode='w', header=True, index=False)

    print(f"Centroids appended to {output_file_path}")
